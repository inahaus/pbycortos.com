...
class PHPPaging {
	...
	**/
	...
	/**
	*	Paginado mostrando el efecto de cargando
	*
	*	Valor por default: '0'
	* 
	*   agregado por lucas Sastre
	*
	*	<code>
	* 	var $ajax = 'publicacion';
	*	</code>
	*	@var string
	* 	@version 2.0
	**/
	...
	/**
	*	Setea si el paginado es del frontend o del admin
	*
	*	Valor por default: '0' -> administracion
	*
	*   agregado por lucas Sastre
	*
	*	<code>
	...
	**/
	...
	/**
	*	Verifica si muestra el efecto ajax de cargando
	*
	*	
	*	@param number $num Número de registros por página que se usará
	*	@returns bool
	**/
	function ajax ($ajax) {
		if ($ajax==0) $this->ajax = false;
		else 		
		$this->ajax = true;
	}

	function frontEnd ($frontEnd) {
		if ($frontEnd==0) $this->frontEnd = 0;
		else
		$this->frontEnd = 1;
	}
	...
	*/
	function do_link ($page, $title = false,$content) { // Función modificada version 2.0
		...
		return "<a href=\"$href\"$title$estilo $ajax>$content</a>";
	}
